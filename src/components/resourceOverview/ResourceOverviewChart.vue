...
<script lang="ts">
    ...
    type ResourcesOverviewChartDatasetAggregator = (
        expos: ExpoEvent[],
        battleReports: BattleReport[],
        debridFieldReports: DebrisFieldReport[]
    ) => number;

    export interface ResourcesOverviewChartDataset {
        label: string;
        color: HexColor;
        fill: boolean;
        aggregator: ResourcesOverviewChartDatasetAggregator;
    }

    @Component({})
    export default class ResourcesOverviewChart extends Vue {
        @Prop({ required: true, type: Array as PropType<ResourcesOverviewChartDataset[]>, default: [] })
        private datasets!: ResourcesOverviewChartDataset[];
        ...
        private readonly battleModule = BattleModule;
        private readonly debrisFieldModule = DebrisFieldModule;
        ...
        private initData() {
            const firstTrackedDayExpo = this.expoModule.firstExpo?.date ?? Date.now();
            const firstTrackedDayBattles = this.battleModule.firstReport?.date ?? Date.now();
            const firstTrackedDayDebridFields = this.debrisFieldModule.firstReport?.date ?? Date.now();
            const firstTrackedDay = Math.min(firstTrackedDayExpo, firstTrackedDayBattles, firstTrackedDayDebridFields);
            ...
            const battlesByDay = this.battleModule.byDay;
            const debrisByDay = this.debrisFieldModule.byDay;

            this.fullDatasetsData.push(
                ...this.datasets.map(
                    dataset => this.allDays.map(
                        day => dataset.aggregator(
                            exposByDay[day.getTime()] ?? [],
                            battlesByDay[day.getTime()] ?? [],
                            debrisByDay[day.getTime()] ?? []
                        )
                    )
                )
            );
