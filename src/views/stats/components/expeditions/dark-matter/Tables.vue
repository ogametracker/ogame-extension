...
<script lang="ts">
    ...
    import { ExpeditionEventDarkMatter } from '@/shared/models/v1/expeditions/ExpeditionEvents';
    import { ExpeditionEventSize } from '@/shared/models/v1/expeditions/ExpeditionEventSize';

    @Component({})
    export default class Tables extends Vue {

        private get exposByRange(): ExpeditionEventDarkMatter[][] {
            const expeditions = ExpeditionDataModule.expeditions;
            const exposByRange: ExpeditionEventDarkMatter[][] = _dev_DateRanges.map(() => []);
            expeditions.forEach(expo => {
                if (expo.type != ExpeditionEventType.darkMatter) {
                    return;
                }
        ...
        private get items(): Record<string, any>[] {
            ...
            //TODO: optimize
            return [{
                darkMatter: 'LOCA: Dark Matter', //LOCA:

                ..._dev_DateRanges.map(
                    (_, rangeIndex) => exposByRange[rangeIndex].reduce(
                        (acc, expo) => acc + expo.darkMatter
                        , 0
                    )
                )
            }];
